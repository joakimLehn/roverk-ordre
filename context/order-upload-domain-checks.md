...
Token-ruten og `saveOrderFile` kaller `authorizeOrderUpload` / `validateOrderFileInsert` i `src/lib/order-file.ts`. `GET /ordre/[id]/filer/[fileId]` bruker `canServeOrderFile`. I/O (`requireUser`, `getOrder`, Blob) blir i rute og action.
...
Issue #34 krever suksess- og feilsti i tester. Auth/DB/Blob er utenfor vitest; domenesjekkene er det som avgjør om token eller rad opprettes. Befaringsruten er urørt. UI-PR-en (#38) la inn rutene inlinet; hjelperne er den testbare kontrakten.
