...
- eier tabellene den selv oppretter: `allowed_emails`, `inspections`,
  `inspection_files` og `order_files`. Full CRUD der. Nettsidens kolonner røres
  ikke.
...
- **Filer**: `@vercel/blob`, befarings- og ordrevedlegg (bilder/PDF). Metadata
  og e-postutdrag ligger i Neon (`inspection_files` / `order_files`). Ingen
  forretningsdata i Supabase Storage. Klienten laster direkte til Blob
