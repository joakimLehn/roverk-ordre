...
Inspection-typer og parse/visning ligger i `src/lib/inspection.ts`, ikke i `src/lib/types.ts`. Datobøtter speiles i `src/lib/inspection-groups.ts` ved å kalle `buildGroupOf` (og `today` fra `osloDate`), uten å gjøre `groupByBuildDate` generisk.
